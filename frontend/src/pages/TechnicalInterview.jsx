...
const TechnicalInterview = () => {
  ...
  if (!session) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <LoadingSpinner text="Initializing Technical Interview Engine..." size="large" />
      </div>
    );
  }

  const { questions, currentIndex, setup, interviewId, answers } = session;
  const currentQuestion = questions[currentIndex] || { text: 'Loading question...' };
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      if (isLastQuestion) {
        // 2. Final Question -> Trigger Evaluation
      ...
      console.error(err);
      setErrorMsg('Failed to record answer. Please try again.');
    } finally {
  ...
  const handleTimeUp = () => {
    if (!currentAnswer.trim()) {
      setCurrentAnswer('// Candidate ran out of time before completing response.');
    }
  };
    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-12">
      ...
      <div className="p-4 rounded-2xl bg-[#111827] border border-gray-800 flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-white">Technical Coding Round</h3>
      ...
      {/* Progress Bar */}
      <ProgressBar current={currentIndex + 1} total={questions.length} label="Technical Round Progress" />
      ...
      <AnswerBox
        ...
        mode="code"
        placeholder={`// Type your technical answer or write pseudocode/code snippets for ${setup?.technology}...`}
      />
    </div>
  );
};
